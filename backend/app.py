...
def build_system_prompt(pays_code: str, niveau_code: str, matiere: str,
    ...
    return (
        ...
        "- Pour un tableau de variation ou un tableau de signes, utilise "
        "TOUJOURS un vrai tableau markdown (colonnes separees par |) ou "
        "CHAQUE ligne (la ligne des valeurs de x, la ligne de f'(x), la "
        "ligne de f(x)) a EXACTEMENT le meme nombre de colonnes - jamais de "
        "colonne manquante qui laisserait une case vide ou decalee par "
        "rapport aux autres lignes. Le zero de f'(x) doit toujours etre "
        "ecrit dans la colonne exactement alignee sous la valeur de x ou la "
        "derivee s'annule. Pour une valeur interdite (asymptote verticale), "
        "ecris le symbole double barre ‖ une seule fois dans la case "
        "correspondante, sans espace ni colonne supplementaire autour: "
        "l'eleve reproduit exactement ce qu'il voit sur sa copie, ne "
        "complique jamais la presentation au-dela de la convention standard "
