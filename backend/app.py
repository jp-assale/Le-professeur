...
def build_system_prompt(pays_code: str, niveau_code: str, matiere: str,
    ...
    return (
        ...
        "- Un domaine de definition avec des valeurs exclues doit toujours "
        "etre une notation LaTeX complete et valide entre signes dollar, "
        "jamais un melange de texte brut et de code LaTeX mal ferme. "
        "Exemple correct: $\\mathbb{R} \\setminus \\{-1, 1\\}$ (jamais "
        "quelque chose comme '}\\R \\setminus {-1, 1}$' avec des accolades "
        "ou des signes dollar manquants/en trop).\n"
        "- Pour un tableau de variation ou un tableau de signes, n'utilise "
        "JAMAIS un tableau markdown (les colonnes ne s'alignent pas "
        "correctement pour ce type de contenu) - utilise TOUJOURS un "
        "tableau LaTeX (environnement array) entre doubles signes dollar, "
        "sur ce modele exact (a adapter selon les valeurs reelles de "
        "l'exercice, en gardant la meme structure) :\n"
        "$$\\begin{array}{c|ccccccccc}\n"
        "x & -\\infty & & -1 & & 0 & & 1 & & +\\infty \\\\\n"
        "\\hline\n"
        "f'(x) & & + & \\Vert & - & 0 & + & \\Vert & + & \\\\\n"
        "\\hline\n"
        "f(x) & & \\nearrow & \\Vert & \\searrow & -1 & \\nearrow & \\Vert "
        "& \\nearrow &\n"
        "\\end{array}$$\n"
        "Regles strictes : chaque ligne (x, f'(x), f(x)) doit avoir "
        "EXACTEMENT le meme nombre de '&' que les autres lignes - compte-"
        "les avant d'envoyer ta reponse, et laisse une case simplement "
        "vide entre deux '&' plutot que de supprimer une colonne. Adapte "
        "le nombre de colonnes au nombre reel de valeurs interdites et de "
        "racines de f'(x) de l'exercice: un exercice avec un seul point "
        "critique et aucune valeur interdite n'aura que 3 colonnes de "
        "valeurs, par exemple 'x & -\\infty & a & +\\infty \\\\ f'(x) & + & "
        "0 & - \\\\ f(x) & \\nearrow & \\text{max} & \\searrow'. Utilise "
        "\\Vert pour une valeur interdite (asymptote verticale), "
        "\\nearrow / \\searrow pour les fleches de variation, jamais "
        "d'autre symbole.\n"
        "- Quand une courbe aiderait a comprendre une fonction (etude de "
        "fonction, trace demande par l'eleve, comparaison de courbes...), "
        "inclus un bloc de code special au format exact suivant (sur ses "
        "propres lignes, avec les balises ``` ) pour que l'appli trace le "
        "graphique automatiquement - ne dis jamais que tu ne peux pas "
        "dessiner de courbe, utilise ce bloc a la place :\n"
        "```plot\n"
        "f(x) = x^3 - 1\n"
        "domaine: [-3, 3]\n"
        "```\n"
        "Regles strictes pour ce bloc : la variable est toujours x, la "
        "multiplication est TOUJOURS explicite avec * (ecris 2*x, jamais "
        "2x), les fonctions disponibles sont sin, cos, tan, sqrt, abs, exp, "
        "ln, log (base 10), les constantes pi et e sont disponibles, et le "
